...
async function guardarEnNube(proyData, ventas) {
  try { await setDoc(doc(db, "binda", "estado"), { proyData, ventas }); }
...
// ── PDF Generator ─────────────────────────────────────────────
function generarPDF(proyData, ventas) {
  ...
  pdf.rect(0, H * 0.35, W, H * 0.3, "F");

  // Línea decorativa
  pdf.setDrawColor(184, 134, 11);
  pdf.setLineWidth(1.2);
  pdf.line(30, H * 0.32, W - 30, H * 0.32);
  pdf.line(30, H * 0.65, W - 30, H * 0.65);
  ...
  pdf.setFontSize(18);
  pdf.text("COMERCIAL Y PROYECTOS", W / 2, H * 0.50, { align: "center" });
  ...
  pdf.setFontSize(8);
  pdf.text(`${proyectos.length} PROYECTOS`, W / 2, H * 0.72, { align: "center" });

  // ── PÁGINAS POR PROYECTO ─────────────────────────────────────
  proyectos.forEach((nombre) => {
    const d  = proyData[nombre];
    ...
    pdf.rect(0, H * 0.38, W, H * 0.24, "F");

    pdf.setDrawColor(r, g, b);
    pdf.setLineWidth(0.8);
    pdf.line(20, H * 0.36, W - 20, H * 0.36);
    pdf.line(20, H * 0.64, W - 20, H * 0.64);
    ...
    pdf.setFontSize(11);
    pdf.setTextColor(255, 255, 255);
    pdf.text(d.fase.toUpperCase(), W / 2, H * 0.55, { align: "center" });
    ...
    kpis.forEach((k, i) => {
      const x = 20 + i * kpiW;
      pdf.setFillColor(30, 41, 59);
      pdf.roundedRect(x, kpiY, kpiW - 4, 22, 2, 2, "F");
      pdf.setTextColor(r, g, b);
      pdf.setFontSize(13);
      pdf.setFont("helvetica", "bold");
    ...
    pdf.setTextColor(150, 150, 150);
    pdf.text(fechaHoy, W - 10, 10, { align: "right" });
    ...
    kpiData.forEach((k, i) => {
      ...
      pdf.roundedRect(kx, ky, kW - 3, 15, 1.5, 1.5, "F");
      pdf.setDrawColor(r, g, b);
      pdf.setLineWidth(0.4);
      pdf.line(kx, ky, kx, ky + 15);
      pdf.setTextColor(r, g, b);
      pdf.setFontSize(11);
      pdf.setFont("helvetica", "bold");
    ...
    // Barra de avance de ventas
    pdf.setFillColor(230, 230, 230);
    pdf.roundedRect(10, y, W - 20, 5, 2, 2, "F");
    pdf.setFillColor(r, g, b);
    pdf.roundedRect(10, y, (W - 20) * (v.avanceVentas / 100), 5, 2, 2, "F");
    ...
    SECCIONES.forEach(seccion => {
      ...
      pdf.setFillColor(r, g, b);
      pdf.roundedRect(10, y, W - 20, 8, 1, 1, "F");
      pdf.setTextColor(255, 255, 255);
      pdf.setFontSize(8);
      pdf.setFont("helvetica", "bold");
      ...
      autoTable(pdf, {
        ...
        didParseCell: (data) => {
          if (data.section === "body" && data.column.index === 2) {
            ...
            if (estado === "Completado") { data.cell.styles.textColor = [26, 122, 60]; data.cell.styles.fontStyle = "bold"; }
            else if (estado === "En Proceso") { data.cell.styles.textColor = [138, 96, 0]; data.cell.styles.fontStyle = "bold"; }
            else if (estado === "Bloqueado") { data.cell.styles.textColor = [185, 28, 28]; data.cell.styles.fontStyle = "bold"; }
          }
          if (data.section === "body" && data.column.index === 0 && data.cell.raw) {
            const estado = data.row.raw[2];
            if (estado === "Completado") data.cell.styles.textColor = [150, 150, 150];
          }
        },
  ...
  const nombreArchivo = `Acta_Comite_Binda_${new Date().toLocaleDateString("es-PE").replace(/\//g,"-")}.pdf`;
    ...
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.4)", zIndex:1000, display:"flex", alignItems:"center", justifyContent:"center", padding:16 }}>
      <div style={{ background:"#fff", border:"1px solid #e5e7eb", borderRadius:16, width:"100%", maxWidth:400, boxShadow:"0 20px 60px rgba(0,0,0,0.15)" }}>
        ...
        <div style={{ padding:24, display:"flex", flexDirection:"column", gap:14 }}>
          <div>
            <label style={{ fontSize:11, color:"#888", letterSpacing:1, textTransform:"uppercase", display:"block", marginBottom:5 }}>Fase actual</label>
          ...
          <div>
            <label style={{ fontSize:11, color:"#888", letterSpacing:1, textTransform:"uppercase", display:"block", marginBottom:5 }}>Fecha fin de obra</label>
            <input value={form.finObra} onChange={e => setForm(f => ({ ...f, finObra: e.target.value }))}
              placeholder="ej: 15/05/2026"
              style={{ width:"100%", border:"1px solid #e5e7eb", borderRadius:8, padding:"9px 12px", fontSize:13, boxSizing:"border-box", color:"#111" }} />
          </div>
          <div>
            <label style={{ fontSize:11, color:"#888", letterSpacing:1, textTransform:"uppercase", display:"block", marginBottom:5 }}>Fecha fin de ventas</label>
            <input value={form.finVentas} onChange={e => setForm(f => ({ ...f, finVentas: e.target.value }))}
              placeholder="ej: 21/07/2026"
              style={{ width:"100%", border:"1px solid #e5e7eb", borderRadius:8, padding:"9px 12px", fontSize:13, boxSizing:"border-box", color:"#111" }} />
          </div>
          <button onClick={() => onSave(form)} style={{ background:color, color:"#fff", border:"none", borderRadius:8, padding:"12px", cursor:"pointer", fontWeight:700, fontSize:14 }}>
            Guardar cambios
          </button>
        </div>
      </div>
    ...
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.4)", zIndex:1000, display:"flex", alignItems:"center", justifyContent:"center", padding:16 }}>
      <div style={{ background:"#fff", border:"1px solid #e5e7eb", borderRadius:16, width:"100%", maxWidth:460, boxShadow:"0 20px 60px rgba(0,0,0,0.15)" }}>
        ...
        <div style={{ padding:24, display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
          {[
            { k:"avanceVentas", label:"% Avance ventas" }, { k:"deptos", label:"Dptos vendidos" },
            { k:"porVender", label:"Dptos por vender" },   { k:"total", label:"Total dptos" },
            { k:"estac", label:"Estac. vendidos" },        { k:"porVenderEstac", label:"Estac. por vender" },
            { k:"totalEstac", label:"Total estacionamientos" }, { k:"velocidad", label:"Velocidad (u/mes)" },
...
function ModalTarea({ tarea, color, seccion, proyectoNombre, onSave, onDelete, onClose }) {
  const [form, setForm] = useState(tarea ? { ...tarea, seccion } : {
    id: Date.now(), tarea:"", responsable:"Roy", estado:"En Proceso", progreso:0, vencimiento:"", nota:"", seccion: seccion||"GESTIÓN"
  });
  ...
  return (
    <div>
      <div style={{ display:"flex", justifyContent:"flex-end", gap:10, marginBottom:16 }}>
        <button onClick={onEditarProyecto} style={{ background:"#f3f4f6", border:"1px solid #e5e7eb", borderRadius:8, padding:"8px 16px", cursor:"pointer", fontSize:13, color:"#374151", fontWeight:600 }}>
          ✎ Editar proyecto
        </button>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(150px, 1fr))", gap:12, marginBottom:24 }}>
        {[
          ...
          { label:"Dptos vendidos",  valor:v.deptos,             color },
...
function TabTareas({ data, color, onEdit, onNueva }) {
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20, flexWrap:"wrap", gap:10 }}>
        <div style={{ display:"flex", gap:10, flexWrap:"wrap" }}>
          {[
            ...
            { label:"Tareas urgentes",val:urgentes,                                                      c:urgentes>0?"#b91c1c":"#9ca3af" },
...
export default function App() {
  ...
  function guardarProyecto(form) { setProyData(prev => ({ ...prev, [activo]: { ...prev[activo], ...form } })); setModalProyecto(false); }
    ...
    <div style={{ minHeight:"100vh", background:"#f3f4f6", fontFamily:"'Segoe UI', system-ui, sans-serif", color:"#111" }}>
      <div style={{ background:"#fff", borderBottom:"1px solid #e5e7eb", padding:"14px 28px", display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:12 }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:12, flexWrap:"wrap" }}>
          ...
          <button onClick={() => generarPDF(proyData, ventas)} style={{ background:data.color, color:"#fff", border:"none", borderRadius:8, padding:"10px 20px", cursor:"pointer", fontWeight:700, fontSize:13 }}>
            ⬇ Descargar Acta
          </button>
        </div>
      </div>
      ...
      <div style={{ display:"flex", height:"calc(100vh - 61px)" }}>
        <div style={{ width:220, minWidth:220, background:"#fff", borderRight:"1px solid #e5e7eb", overflowY:"auto", padding:"16px 10px" }}>
          ...
          {proyectos.map(p => {
            const d=proyData[p]; const v=ventas[p];
            ...
            return (
              <div key={p} onClick={()=>{setActivo(p);setTabActiva("tareas");}} style={{
                background:esActivo?`${d.color}12`:"transparent", border:`1px solid ${esActivo?d.color:"transparent"}`,
        ...
        <div style={{ flex:1, overflowY:"auto", padding:"24px 28px" }}>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(140px, 1fr))", gap:12, marginBottom:24 }}>
            {[
              ...
              { label:"Dptos vendidos",  valor:`${venta.deptos}/${venta.total}`, color:"#374151" },
          ...
          <div style={{ background:"#fff", border:"1px solid #e5e7eb", borderRadius:14, overflow:"hidden" }}>
            ...
            <div style={{ padding:"20px 24px" }}>
              {tabActiva==="tareas" && <TabTareas data={data} color={data.color} onEdit={(t,sec)=>{setSeccionModal(sec);setModalTarea(t);}} onNueva={abrirNueva} />}
              {tabActiva==="ventas" && <TabVentas nombre={activo} ventas={venta} data={data} color={data.color} onEditar={()=>setModalVentas(true)} onEditarProyecto={()=>setModalProyecto(true)} />}
            </div>
          </div>
        </div>
      </div>
      ...
      {modalProyecto && <ModalEditarProyecto nombre={activo} data={data} color={data.color} onSave={guardarProyecto} onClose={()=>setModalProyecto(false)} />}
    </div>
  );
}
