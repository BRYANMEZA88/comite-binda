...
// ── Guardar y cargar desde Firestore ─────────────────────────
async function guardarEnNube(proyData, ventas) {
  try {
    await setDoc(doc(db, "binda", "estado"), { proyData, ventas });
  } catch (e) {
...
function GuardadoIndicador({ estado }) {
  const estilos = {
    guardando: { bg:"#fff3cd", text:"#856404", msg:"Guardando..." },
    guardado:  { bg:"#e6f4ec", text:"#1a7a3c", msg:"✓ Guardado en la nube" },
    error:     { bg:"#fde8e8", text:"#b91c1c", msg:"Error al guardar" },
  };
  const s = estilos[estado];
  if (!s) return null;
  return (
    <div style={{ background:s.bg, color:s.text, border:`1px solid ${s.text}44`, borderRadius:8, padding:"6px 14px", fontSize:12, fontWeight:600 }}>
      {s.msg}
    </div>
  );
}
...
export default function App() {
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#f3f4f6", fontFamily:"'Segoe UI', system-ui, sans-serif", color:"#111" }}>
        ...
        <div style={{ flex:1, overflowY:"auto", padding:"24px 28px" }}>
          ...
          <div style={{ background:"#fff", border:"1px solid #e5e7eb", borderRadius:14, overflow:"hidden" }}>
            ...
            <div style={{ padding:"20px 24px" }}>
              {tabActiva === "tareas" && (
                <TabTareas data={data} color={data.color}
                  onEdit={(t, sec) => { setSeccionModal(sec); setModalTarea(t); }}
                  onNueva={abrirNueva} />
              )}
              {tabActiva === "ventas" && (
                <TabVentas nombre={activo} ventas={venta} data={data} color={data.color} onEditar={() => setModalVentas(true)} />
              )}
            </div>
          </div>
        </div>
      ...
      {modalVentas && (
        <ModalVentas nombre={activo} ventas={venta} color={data.color} onSave={guardarVentas} onClose={() => setModalVentas(false)} />
      )}
